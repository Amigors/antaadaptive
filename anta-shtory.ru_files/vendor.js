...
(function($)
{
    ...
    $.widget('jqAmpUI.spinner', {
        ...
        options: {
            ...
            buttons: {
                position: 'inside', // inside | outside | insideStacked | outsideStacked
                up: {
                    icon: defaultIcons[0],
                    title: null
                },
        ...
        _create: function()
        {
            ...
            this.uiSpinner = uiSpinner = element.addClass('ui-spinner-input ui-corner-all')
                .attr({
                    'autocomplete': 'off',
                    'role': 'spinbutton',
                ...
                .bind({
                    ...
                    'blur.spinner': function(event)
                    {
                        ...
                        if ($this._compare($this._previous, val) !== 0)
            ...
            if (options.disabled || element._prop('disabled'))
                this._setOption('disabled', true);

            if (options.readonly || element._prop('readonly'))
        ...
        destroy: function()
        {
            ...
            this.element.unbind('.spinner')
                ...
                .removeAttr('disabled autocomplete role aria-valuemin aria-valuemax aria-valuenow');
        ...
        _setupButtons: function()
        {
            ...
            var buttons = this.options.buttons, $this = this, stacked = false, inside = true;
            if (!buttons)
                return;
            ...
            if (buttons.position)
            {
                ...
                if (buttons.position === 'outside' || buttons.position === 'outsideStacked')
                    inside = false;
            }

            if (inside)
                this.uiSpinner.addClass('ui-widget-content ui-corner-all ui-spinner-inside').children('.ui-spinner-input').removeClass('ui-corner-all');
            else
                this.uiSpinner.addClass('ui-spinner-outside').removeClass('ui-widget-content ui-corner-all').children('.ui-spinner-input').addClass('ui-corner-all');
            ...
            if (buttons && buttons.up && buttons.up.title)
                this.uiSpinner.children('.ui-spinner-up').attr('title', buttons.up.title);

            if (buttons && buttons.down && buttons.down.title)
                this.uiSpinner.children('.ui-spinner-down').attr('title', buttons.down.title);
        ...
        _removeButtons: function()
        {
            ...
            this.uiSpinner.removeClass('ui-spinner-inside ui-spinner-outside ui-spinner-stacked ui-spinner-adjacent ui-widget-content ui-corner-all')
                .addClass('ui-spinner-nobuttons')
                .children('.ui-spinner-input').addClass('ui-corner-all').end()
                .children('.ui-spinner-button').remove();
        ...
        _setupKeyboard: function()
        {
            ...
            this.element.bind({
                'keydown.spinner': function(event)
                {
                    ...
                    if (keyboard === true || !keyboard.allowed || $this._isAllowedKey(event.which, event))
                        return;

                    if ($.isFunction(keyboard.allowed) && keyboard.allowed.apply($this, [event.which, event]))
                        return;

                    else if ($.isArray(keyboard.allowed) && keyboard.allowed.indexOf(event.which) >= 0)
                        return;
        ...
        _getAmount: function(obj, ctrl, shift)
        {
            ...
            if (ctrl)
                return obj.ctrlAction ? this._actionAmount(obj.ctrlAction) : this._getHalfStep();

            if (shift)
                return obj.shiftAction ? this._actionAmount(obj.shiftAction) : this._getPage();
        ...
        _setValue: function(val, raiseChange, noMinMaxCheck)
        {
            ...
            var isEmpty = this._isEmptyVal(val), formattedVal = this._format(val), triggerChange = raiseChange && formattedVal !== this.element.val();
        ...
        _setOption: function(key, value)
        {
            ...
            switch (key)
            {
                case 'disabled':
                    ...
                    if (options.buttons)
                        uiSpinner.children('.ui-spinner-button').button(value ? 'disable' : 'enable');
                    break;
                ...
                case 'min':
                    ...
                    if (min !== false && this._compare(this.value(), min) < 0)
                        this._setValue(min, true, true);
                    break;

                case 'max':
                    options.max = value;
                    var max = this._maxVal();
                    if (max !== false && this._compare(this.value(), max) > 0)
                        this._setValue(max, true, true);
                    break;
        ...
        value: function(val)
        {
            ...
            if (val === undefined)
            {
                ...
                if (options.empty !== undefined && options.empty !== false)
                    this._cloneValue(options.empty);

                return null;
            }

            if (!options.disabled)
                this._setValue(this._parse(val), true, false);
    ...
    $.isControlKey = function(keyCode)
    {
        ...
        return (keyCode <= 47 && keyCode != 32)
            ...
            || (keyCode >= 112 && [186, 187, 188, 189, 190, 191, 192, 219, 220, 221, 222].indexOf(keyCode) === -1);
    }
...
(function (factory) {
    ...
    function handler(event) {
        ...
        //firefox
        if (window.navigator.userAgent.toLowerCase().match(/firefox/gi) !== null) { deltaY *= 40; deltaX *= 40; delta *= 40; }

        //touchpad
        /*
        if (orgEvent.deltaMode === 0) {
            deltaY = Math.max(-120, Math.min(deltaY / 5, 120));
            deltaX = Math.max(-120, Math.min(deltaX / 5, 120));
            delta = Math.max(-120, Math.min(delta / 5, 120));
        }*/
